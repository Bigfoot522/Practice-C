...
int main_pointer_pj(void) {
	...
	cursor = arrayFish; // cursor[0] .. crsor[1]

	startTime = clock(); // 현재 시간을 millisecond (1000분의 1초) 단위로 변환
	while (1) { // 1을 넣으면 무한 반복
		printFishes();
		...
		totalElapsedTime = (clock() - startTime) / CLOCKS_PER_SEC; // 밀리세컨드를 초단위로 바꿔줌
		printf("총 경과 시간 : %ld 초\n", totalElapsedTime);
		...
		// 레벨 업을 할 건지 확인 (레벨 업은 20초마다 한번씩 수행)
